...
Мне привычно работать не с одной технологией, а на стыке разнородных сред. В
моём опыте есть Microsoft Active Directory в корпоративном контуре DetMir,
прокси-серверы, Linux-почта на Sendmail, qmail, Postfix, Exim и Dovecot, а также
развёртывание и дальнейшая поддержка доменов `sevnb.ru` и `снб.рф`.
...
- **Microsoft Active Directory:** сопровождение AD в контуре DetMir и работа с
  доменной средой как частью общей инфраструктуры;
- **прокси-серверы:** работа с сервисами, через которые проходит доступ
  пользователей и систем;
- **почтовые серверы:** практическая работа с Sendmail, qmail, Postfix, Exim и
  Dovecot — от разных поколений почтового стека до их сопровождения;
- **домены и связанные сервисы:** развёртывание и поддержка `sevnb.ru` и
  кириллического домена `снб.рф`;
- **смешанная инфраструктура:** поиск причин неисправностей на границе Windows,
  Linux, сети, DNS, почты, телефонии и прикладных сервисов.

Я намеренно не публикую здесь внутренние схемы, адреса, учётные данные и рабочие
конфигурации. На собеседовании могу рассказать о подходе к эксплуатации,
диагностике и безопасным изменениям без раскрытия закрытой информации.

Подробнее: [опыт работы с разнородной инфраструктурой](docs/INFRASTRUCTURE_EXPERIENCE_RU.md).

## Чем я автоматизирую работу

- **Go** — backend-сервисы и внутренние инструменты с API, проверками и
  предсказуемым развёртыванием одним исполняемым файлом;
- **Rust** — эксплуатационные CLI, проверки состояния и обработка данных, где
  важны надёжность, скорость и контролируемое потребление ресурсов;
- **Bash и sh** — диагностика, установочные сценарии, служебные обёртки и
  интеграция с системными командами;
- **Ansible** — повторяемое развёртывание и настройка Linux- и Windows-узлов без
  ручного воспроизведения длинной последовательности действий.

Я выбираю инструмент по задаче. Небольшой прозрачный shell-сценарий не нужно
превращать в отдельный сервис, а критичную долгоживущую логику не стоит оставлять
в хрупкой цепочке команд.
