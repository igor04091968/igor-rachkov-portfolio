# Asterisk SSH WebUI: управление телефонией через SSH

В этом проекте знания эксплуатации Asterisk превращены в рабочий инструмент для
...
Телефония интегрирована с ejabberd и клиентами Miranda IM. Dialplan отправляет
XMPP-уведомления о входящих, пропущенных и занятых вызовах, сообщает о состоянии
транков и переадресации. Звонок по клику из Miranda передаёт XMPP-запрос в
Asterisk: JID инициатора сопоставляется с внутренним номером, после чего
Originate соединяет его с выбранным абонентом. Аналогично из сообщения можно
запустить телефонную конференцию.

## Проверка качества

Backend проверяется тестами Go, форматированием и статическим анализом. Основные
сценарии интерфейса проходят в Playwright.
